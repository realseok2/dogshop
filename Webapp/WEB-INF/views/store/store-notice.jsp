                ...
                <tbody>
                  	<c:forEach var ="i" begin="0" end="${fn:length(sMap.boardList)}">
                  		<c:if test="${i != fn:length(sMap.boardList) }">
                    		<tr><th scope="row">${i+1 }</th><td class="text-left px-5"><a href="#">${sMap.boardList[i].title }</a></td><td>${sMap.boardList[i].regdate }</td><td>운영자</td><td>${sMap.boardList[i].hit }</td></tr>
                		</c:if>
                	</c:forEach>
                </tbody>
              </table>
